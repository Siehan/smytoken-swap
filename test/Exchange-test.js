...
describe('Exchange', () => {
  ...
  beforeEach(async () => {
    ...
    exchange = await Exchange.deploy(smyToken.address);
  ...
  describe('addLiquidity', async () => {
    it('adds liquidity', async () => {
      await smyToken.approve(exchange.address, toWei(200));
    ...
    it('allows zero amounts', async () => {
      await smyToken.approve(exchange.address, 0);
  ...
  describe('getTokenAmount', async () => {
    it('returns correct token amount', async () => {
      await smyToken.approve(exchange.address, toWei(2000));
  ...
  describe('ethToTokenSwap', async () => {
    ...
    it('transfers at least min amount of tokens', async () => {
      ...
      expect(fromWei(userBalanceAfter - userBalanceBefore)).to.equal('-1.0004877520006021');

      const userTokenBalance = await smyToken.balanceOf(user.address);
      ...
      const exchangeTokenBalance = await smyToken.balanceOf(exchange.address);
  ...
  describe('tokenToEthSwap', async () => {
    beforeEach(async () => {
      await smyToken.transfer(user.address, toWei(2));
      await smyToken.connect(user).approve(exchange.address, toWei(2));
    ...
    it('transfers at least min amount of tokens', async () => {
      ...
      const userBalanceAfter = await getBalance(user.address);
      expect(fromWei(userBalanceAfter - userBalanceBefore)).to.equal('0.9987649429999452');
